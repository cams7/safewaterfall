...
import br.com.cams7.safewaterfall.swsensor.arduino.model.repository.CurrentStatusRepository;
...
import br.com.cams7.safewaterfall.swsensor.arduino.util.Bytes;
...
public abstract class ArduinoServiceImpl implements ArduinoService, Runnable, SerialPortEventListener {
	 ...
	 * 
	 * @param serialPort - Porta COM que sera utilizada para enviar os dados para o
	 *                   Arduino
	...
	protected void addCurrentStatus(Arduino arduino) {
		...
		if (!repository.existsById(id)) {
			...
			currentStatus = repository.findById(id).orElseThrow(() -> new ArduinoException(
	...
	private void receiveDataBySerial(Arduino arduino) {
		...
		case RESPONSE_RESPONSE:
			switch (arduino.getEvent()) {
			case EXECUTE:
				short pinValue = sendResponse(arduino.getPinType(), arduino.getPin(),
	...
	protected void sendPinDigitalUSART(ArduinoStatus status, byte digitalPin, boolean pinValue) {
		ArduinoUSART arduino = new ArduinoUSART(status, ArduinoPinType.DIGITAL, digitalPin,
				(short) (pinValue ? 0x0001 : 0x0000));
