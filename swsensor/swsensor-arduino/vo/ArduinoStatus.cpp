...
const transmitter ArduinoStatus::ARDUINO_DEVICE = 0x00;
...
ArduinoStatus::ArduinoStatus() :
		ArduinoPin(), _transmitterValue(ARDUINO_DEVICE), _statusValue(SEND), _eventValue(
				EXECUTE) {
}

ArduinoStatus::ArduinoStatus(status statusValue, event eventValue,
		pin_type pinType, uint8_t pin) :
		ArduinoPin(pinType, pin), _transmitterValue(ARDUINO_DEVICE), _statusValue(
				statusValue), _eventValue(eventValue) {
}

ArduinoStatus::ArduinoStatus(status statusValue, event eventValue,
		ArduinoPin* pin) :
		_transmitterValue(ARDUINO_DEVICE), _statusValue(statusValue), _eventValue(
				eventValue) {
