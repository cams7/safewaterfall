...
public class SirenEndpoint extends BaseEndpoint<AppSirenVO> {
	...
	public void changeStatus(@ApiParam("Sensor") @Valid @RequestBody AppSensorVO appSensor) {
		...
		List<AppSirenVO> sirens = StreamSupport.stream(appSirenService.findAll().spliterator(), false)
				.filter(s -> DEVICE_ID.equals(s.getSirenId())).collect(Collectors.toList());

		AppSirenVO appSiren;
		if (sirens.stream().anyMatch(s -> sensorId.equals(s.getId()))) {
			appSiren = sirens.stream().filter(s -> sensorId.equals(s.getId())).findAny().get();
		} else {
			appSiren = new AppSirenVO(sensorId, DEVICE_ID);
			changeSirenStatus = sirens.stream().noneMatch(s -> !sensorId.equals(s.getId()));
		}

		if (!changeSirenStatus) {
			boolean isAnotherActiveSensor = !active
					&& sirens.stream().anyMatch(s -> s.isActive() && !sensorId.equals(s.getId()));
		...
		if (changeSirenStatus || sirens.stream().anyMatch(s -> sensorId.equals(s.getId()) && active != s.isActive()))
