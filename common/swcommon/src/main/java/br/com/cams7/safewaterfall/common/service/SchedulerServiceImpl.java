...
public class SchedulerServiceImpl implements SchedulerService {
  ...
  public void register(JobDetail jobDetail, CronTrigger cronTrigger) {
    ...
    try {
      ...
      if (scheduler.getScheduler().checkExists(cronTrigger.getKey())) {
        ...
        // Option three:
        log.log(Level.INFO, "Quartz Scheduler keep the trigger status {0}", new Object[] {cronTrigger.getKey()});
      } else {
        scheduler.getScheduler().scheduleJob(jobDetail, cronTrigger);
        log.log(Level.INFO, "Quartz Scheduler register new trigger {0}", new Object[] {cronTrigger.getKey()});
      }
    } catch (SchedulerException | RuntimeException e) {
  ...
  public void reschedule(CronTrigger cronTrigger) {
    try {
      log.log(Level.INFO, "Reschedule trigger {0}", new Object[] {cronTrigger.getKey()});
      log.log(Level.INFO, "The new schedule is {0}", new Object[] {cronTrigger.getCronExpression()});
  ...
  public void pause(CronTrigger cronTrigger) {
    try {
      log.log(Level.INFO, "Pause trigger {0}", new Object[] {cronTrigger.getKey()});
